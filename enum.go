...
MemoryArea word is big end
	...
	@Enum(areaName string, dataType string, code byte, max uint16, offset uint16, oldCode byte, oldMax uint16, oldOffset uint16, size int) {
		...
		WWordFs("WR", "WordFs", 0xF1, 511, 0, 0x0, -1, -1, 4) 	 		// Work Area Word with forced status
		HWordFs("HR", "WordFs", 0xF2, 511, 0, 0x0, -1, -1, 4) 	 		// Holding Bit Area Word with forced status
